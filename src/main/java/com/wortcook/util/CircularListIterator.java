...
public interface CircularListIterator<T> extends PositionalListIterator<T>, CountingIterator<T>{
    ...
    public class CircularListIteratorImpl<T> implements CircularListIterator<T>{
        ...
        public boolean hasNext() {
            ...
            (stepCount < maxAllowedSteps)? true : hasNextNoWrap();
        }
        ...
        public boolean hasPrevious() {
            return !elements.isEmpty() && 
                (stepCount < maxAllowedSteps)? true : hasPreviousNoWrap();
        }
        ...
        @Override
        public void addPrevious(T element) {
            add(element);
        }

        @Override
        public boolean hasExceeded() {
            return stepCount >= maxAllowedSteps;
        }
    }
    ...
    public static <T> Iterable<T> iterableOf(final List<T> elements, final int index){
        return new Iterable<T>(){
            @Override
            public Iterator<T> iterator() {
                return new CircularListIteratorImpl<>(new ArrayList<>(elements), index);
            }
        };
    }
    ...
    public static <T> Iterable<T> iterableOf(final List<T> elements, final int index, final int maxSteps){
        return new Iterable<T>(){
            @Override
            public Iterator<T> iterator() {
                return new CircularListIteratorImpl<>(new ArrayList<>(elements), index, maxSteps);
            }
        };
    }
}
